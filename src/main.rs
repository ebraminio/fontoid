// Mostly copied from https://github.com/RazrFalcon/ttf-parser/blob/master/examples/font2svg.rs
...
async fn endpoint(param: Query<HashMap<String, String>>) -> impl IntoResponse {
    // if !param.contains_key("font") && !param.contains_key("text") {
    //     return Ok((
    //         headers,
    //         String::from("Fontoid, https://github.com/ebraminio/Fontoid"),
    //     ));
    // }

    let font = param.get("font").unwrap();

    let result = process(Args {
        ttf_path: PathBuf::from(format!("/System/Library/Fonts/Supplemental/{}.ttf", font)),
        variations: param.get("variations").map(|x| parse_variations(x).unwrap_or_default()).unwrap_or_default(),
        colr_palette: param.get("colr-palette").map(|x| x.parse::<u16>().unwrap_or_default()).unwrap_or_default(),
    })
    .unwrap_or_else(|_| String::from("Error"));

    ([(header::CONTENT_TYPE, "image/svg+xml")], result)
}

#[tokio::main]
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    let router = axum::Router::new().route("/", axum::routing::get(endpoint));
    let address = "0.0.0.0";
    let port = 8000;
...
fn process(args: Args) -> Result<(String), Box<dyn std::error::Error>> {
    println!("{}", args.ttf_path.as_path().to_str().unwrap());
